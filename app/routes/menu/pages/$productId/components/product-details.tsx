import { Clock, Flame, Leaf, Star, Weight } from "lucide-react";
	...
	return (
		<div className="flex flex-col gap-4">
			...
			{product.ingredients?.length > 0 && (
				<div>
					<p className="text-sm font-semibold text-gray-800 mb-2">{t.product.ingredients}</p>
					<p className="text-sm text-gray-500 leading-relaxed">{product.ingredients.join(", ")}</p>
				</div>
			)}
			...
			{product.allergens?.length > 0 && (
				<div>
					<p className="text-sm font-semibold text-gray-800 mb-2">{t.product.allergens}</p>
					<div className="flex flex-wrap gap-1.5">
						{product.allergens.map((a) => (
							<Badge key={a} variant="outline" className="text-xs text-gray-500 border-gray-200">
								{a}
							</Badge>
						))}
					</div>
