...
export async function getCategories(params?: { page?: number; limit?: number; parentId?: number }) {
...
export async function getCategoryChildren(parentId: number) {
	const res = await api.get<ApiResponse<CategoryDto[]>>(`/api/categories/${parentId}/children`);
	return res.data;
}
